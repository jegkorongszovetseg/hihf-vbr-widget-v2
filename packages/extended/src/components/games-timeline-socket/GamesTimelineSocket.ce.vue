...
import Carousel from '../games-timeline/Carousel.vue';
import CarouselItem from '../games-timeline/CarouselItem.vue';
import ExternalSchedule from '../games-timeline/ExternalSchedule.vue';
import { mergeGames, useGameDataService } from '../games-timeline/internal';
import TrayAgain from '../games-timeline/TryAgain.vue';
...
const { resume, pause } = useIntervalFn(execute, REFRESH_INTERVAL);
...
async function handleLiveGames() {
  ...
  for (let i = 0; i < liveGames.length; i++) {
    ...
    const { pause } = useIntervalFn(() => fetchGameData(0, { gameId: id }).then(data => updateGameData(data)), REFRESH_INTERVAL);
  ...
  <I18NProvider v-slot="{ t }" :locale="locale" :messages="messages">
    <Carousel :key="isLoadingDebounced" :initial-index="initialIndex">
      ...
      <template v-else>
        ...
        >
          <Game :game-id="game.id" :resolved-api-key="resolvedApiKey" :game-data="game" :external-game-resolver="externalGameResolver" @navigate-to="navigateTo" />
        </CarouselItem>
        <CarouselItem v-once>
          <ExternalSchedule :external-schedule-url="externalScheduleUrl" :title="t('gamesTimeline.allSchedule')" @navigate-to="navigateTo" />
        </CarouselItem>
